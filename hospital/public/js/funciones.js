...
function buttonsTable(show,edit,deleted,editView=false){	
	...
	if (edit!=''){
    	...
    	btn += '<span class="green"><i class=""></i></span></a></li>';
	}    
    if (deleted!=''){
    	btn += '<li><a href="#" class="tooltip-error" data-rel="tooltip" title="Delete" '+btnDelete+'>';
    	btn += '<span class="red"><i class=""></i></span></a></li>';
    }
    btn += '</ul></div></div>';
    return btn;    
}
...
function destroy(route){
	bootbox.confirm({
		title:'Confirmacisdfgón',
	    message: '¿Está seguro de eliminar el registro?',
	    buttons: {
	        confirm: {label: 'Si, seguro.',className: 'btn-danger'},
	        cancel: {label: 'Nosdfb',className: 'btn-default'}
	    },
