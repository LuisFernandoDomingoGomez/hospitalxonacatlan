...
return redirect()->route('principaln');
	...
	public function guardaindicacionesmedicas(Request $request){
		$Id_Notas= $request->Id_Notas;
		$hoja= $request->hoja;
		$Nombre= $request->Nombre;
		$Apellido_P= $request->Apellido_P;
...
public function altaindicacionesmedicas()
    {    
	...
	if(Session::get('sesionidu')=="")
		...
		{
	$clavequesigue = notas::withTrashed()->orderBy('Id_Imedicas','desc')
								->take(1)
								->get();
	if(count($clavequesigue)==0)
	{
