...
<body>
<div align='center'>
<form class="form-horizontal" action="{{ 'guardaimed' }}" method='POST'>
{{csrf_field()}}
...
<?php 
   ...
   ?>
	...
	<option value = "{{$mostrar['1'] }}">{{ $mostrar['nom_unidad'] }}</option>
	<option value = "{{$mostrar['2'] }}">{{ $mostrar['nom_unidad'] }}</option>
...
<a href="{{URL::action('Hojaindicacionesmedicas@indicacionesmedicas')}}"><input name="button" type="button" class="btn btn-danger btn-lg nover" value="Volver" /> </a>
</div></body>
